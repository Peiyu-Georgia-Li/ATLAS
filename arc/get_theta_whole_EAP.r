...
# CLEAN DATA
###############
data<-read.csv("/store01/nchawla/pli9/llmbenchmark/select_model/gaussian_sampled_arc_response_matrix.csv")#⚠️
...
chunk_indices <- c(seq(106, 736, 105), 842)#⚠️(seq 106 105 737) 842
...
linked_params_list[[1]] <- read.csv(paste0("irt_item_parameters_", 106, ".csv")) #⚠️
...
if (ncol(clean_data) > 0 && parameter_count > 0) {
  ...
  if (length(problem_rows) > 0) {
    ...
    for (row_idx in problem_rows) {
      ...
      try({
        ...
        row_theta <- fscores(mod_fixed, method="EAP", response.pattern=row_data,
                            quadpts=81)
  ...
  theta_df <- data.frame(Model_Name = model_names, Theta_EAP = thetas[, 1], SE = thetas[, 2])
  
  # Save results
  output_file <- "irt_person_scores_EAP_SE.csv"
