# Metabench Theta Calculation
# Script to calculate theta values based on pre-selected items from metabench item ID files
...
# Set the dataset name - Choose one of: "ARC", "GSM8K", "HellaSwag", "TruthfulQA", "Winogrande"
DATASET_NAME <- "GSM8K"

# Set whether to use primary or secondary metabench items
METABENCH_SET <- "secondary" # Options: "primary" or "secondary"


# 3. Define output directory for results
DIRECTORY <- "./"
dir.create(DIRECTORY, recursive = TRUE, showWarnings = FALSE)

# 4. Load response matrix function
load_response_matrix <- function() {
if (DATASET_NAME == "TruthfulQA" || DATASET_NAME == "HellaSwag" || DATASET_NAME == "Winogrande") {
  response_data <- read.csv(paste0("../select_model/gaussian_sampled_", tolower(DATASET_NAME), "_response_matrix_train.csv"), 
                          stringsAsFactors = FALSE, check.names = FALSE)
  return(response_data)
} else {
  response_data <- read.csv(paste0("../select_model/gaussian_sampled_", tolower(DATASET_NAME), "_response_matrix.csv"), 
                          stringsAsFactors = FALSE, check.names = FALSE)
  return(response_data)
}
}
...
load_item_parameters <- function() {
  item_params <- read.csv(paste0("../", tolower(DATASET_NAME), "/irt_item_parameters_combined.csv"), stringsAsFactors = FALSE)
  cat("Loaded", nrow(item_params), "items from parameter file\n")
  cat("First few parameter IDs:", paste(head(item_params$X, 10), collapse=", "), "...\n")
  return(item_params)
}

# 5. Function to load pre-selected items from metabench item IDs CSV
load_preselected_items <- function() {
  # Load the preselected items from CSV
  filename <- paste0("metabench_", tolower(DATASET_NAME), "_item_ids_", METABENCH_SET, ".csv")
  cat("Reading metabench items from:", filename, "\n")
  
  preselected_data <- read.csv(filename, stringsAsFactors = FALSE)
  ...
  cat("Loaded", length(selected_items), "pre-selected items from metabench\n")
  cat("First few items:", paste(head(selected_items, 10), collapse=", "), "...\n")
  
  return(selected_items)
}

# 6. Function to map selected items to parameter matrix
...
# 7. Function to calculate theta based on selected items
calculate_theta <- function(selected_items) {
  ...
  # Check for direct matches without X prefix
  direct_matches <- selected_items[selected_items %in% colnames(response_data)]
  
  # Combine both types of matches
  available_items <- unique(c(available_items, direct_matches))
  ...
  if (length(available_items) == 0) {
    stop("No metabench items found in the response matrix. Check item ID format.")
  }
  
  # Extract the columns we need from the response matrix
  columns_to_extract <- c(1, which(colnames(response_data) %in% available_items))
  ...
  cat("Using", length(item_ids), "items for theta calculation\n")
  ...
  for (col in colnames(response_df)) {
    if (length(unique(response_df[[col]])) == 1) {
      cat("Item", col, "has no variance (all responses are", response_df[1, col], ") - removing from analysis\n")
  ...
  if (length(constant_columns) > 0) {
    ...
    response_df <- response_df[, !(colnames(response_df) %in% constant_columns), drop = FALSE]
    
    # Update item_ids to exclude constant items
    item_ids <- colnames(response_df)
    
    cat("Removed", length(constant_columns), "constant items. Continuing with", length(item_ids), "items.\n")
  ...
  # Output filename
  output_filename <- paste0("metabench_", tolower(DATASET_NAME), "_", METABENCH_SET, "_theta_results_eap.csv")
  
  cat("\n===========================================\n")
  cat("SUMMARY\n")
  cat("===========================================\n")
  cat("Dataset:", DATASET_NAME, "\n")
  cat("Metabench set:", METABENCH_SET, "\n") 
  cat("Number of models processed:", nrow(theta_df), "\n")
  cat("Results saved to:", output_filename, "\n")
  
  # Save results
  write.csv(theta_df, output_filename, row.names = FALSE)
...
main <- function() {
  cat("Starting metabench theta analysis for", DATASET_NAME, "(", METABENCH_SET, ")...\n")
